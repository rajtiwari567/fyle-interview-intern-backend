...
class Assignment(db.Model):
    ...
    def submit(cls, _id, teacher_id, principal: Principal):
        ...
        assertions.assert_valid(assignment.state ==
                                AssignmentStateEnum.DRAFT, "only a draft assignment can be submitted")
    ...
    def grade_assignment(cls, _id, grade, principal: Principal):
        ...
        assertions.assert_valid(
            principal.teacher_id == assignment.teacher_id, "teacher_id does not match")
        assertions.assert_valid(
            grade in ['A', 'B', 'C', 'D'], msg="Invalid Grade", case="grade")
        assertions.assert_valid(
            assignment.state == AssignmentStateEnum.SUBMITTED, "Assignment not yet submitted or already graded")
